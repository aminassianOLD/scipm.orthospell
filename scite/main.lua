-- -*- coding: utf-8 -

-- scipm.orthospell for SciTE
-- Author: Alban Minassian
-- Based on: Urs Eberle orthospell <http://tools.diorama.ch>
-- Based on: Matt White's luahunspell <http://luahunspell.googlecode.com>
-- OS: Windows (for Linux changes must be made)

-- --------------------------------------------------------------------------------------------------------------------------------
-- scipm.orthospell namespace
-- --------------------------------------------------------------------------------------------------------------------------------
scipm.orthospell = {}
scipm.orthospell.config = {
    -- ["key1"] = "1"
};
scipm.orthospell.data = {};

-- global variables
local sciteHome = props["SciteDefaultHome"]
local dictpath = scite_GetProp("scipm.orthospell.dictpath", table.concat({scipm.data.path.scipmchild, "scipm.orthospell", "dict"}, scipm.data.path.sep))
scipm.orthospell.data.dictpath = dictpath
local userdict = scite_GetProp("scipm.orthospell.userdict", nil)
local dictlist = split(scite_GetProp("scipm.orthospell.dictname", "en_US|en_AU|en_CA|en_GB|en_ZA|fr-classique|fr-moderne|fr-reforme1990|fr-toutesvariantes"),"|")
scipm.orthospell.data.dictlist = dictlist
local dictname = dictlist[1]
scipm.orthospell.data.dictname = dictname
local dictCP
local dictlang
local ignoreCAPS = true -- default
local spellIndic = 2
local isoChars
local utfChars
local langChars
local cpMode
local wordRegEx
local dicInit = false
local hspell = false
local toDic, toDoc = true , false
local spChars
local spellChars = props["chars.alpha"]..props["chars.accented"]
local txtChars, htmlChars, texChars
local allText
local ortho = os.getenv("temp").."\\ortho.dic"

-- begin of customization (define your additions and changes here)

-- the following intercept functions allow you to make lexer- or language-dependent
-- changes to the standard processing without changing the main program.
-- place your functions into the corresponding section further down

-- text for the SciTE menu, use utf-8

local lang = {}
lang["en_US"] = "English US"
lang["en-GB"] = "English UK"
lang["de_DE"] = "Deutsch"
lang["de_CH"] = "Deutsch CH"
lang["fr-classique"] = "Fran√ßais classique"
lang["es_ES"] = "Espa√±ol"
lang["it_IT"] = "Italiano"

scipm.orthospell.get_language = function (dic)
	if lang[dic] then lng = lang[dic]
	else lng = dic end
	return lng
end
scipm.orthospell.get_langParam = function(dn)
-- Language settings
	dictCP = "iso"     -- default dictionary code page, redefine below if necessary (see French)
	isoChars = "%a¿-ˇ" -- equal to ISO 8859-1 (Latin 1); redefine in the language section if necessary;
	utfChars = "%a\128-\191\195-\212"  -- UTF-8 up to U+053F (Cyrillic); redefine in the language section if necessary;
	langChars = ""      -- special characters for individual languages such as hyphens and apostrophes
						-- if your ISO 8859-x has letters in the range of 161(xA1) to 191(xBF), add them to langChars
	if dn == "en_US" then
		langChars = "-'"
	elseif dn == "de_DE" then
		langChars = "-"
	elseif dn == "de_CH" then
		langChars = "-"
	elseif dn == "fr-classique" then
		dictCP = "utf"
		langChars = "'í"
	elseif dn == "en-GB" then
		langChars = "-'"
	elseif dn == "it_IT" then
		langChars = "'"
	end
	dictlang = string.sub(dn,1,2)
	props["spell.dictlang"] =  scipm.orthospell.get_language(dn) -- display dictionary in status bar (must be defined)
end

scipm.orthospell.add_userDict = function()
	if not scite_FileExists(userdict) then
		print ("The user dictionary '"..userdict.."' does not exist")
	else
		indic  = io.open(userdict, "r")
		if indic then
			local t = indic:read("*all")
			indic:close()
			local outdic = io.open(ortho,"w")
			outdic:write("999\n")
			outdic:write(t)
			outdic:close()
			hunspell.add_dic(ortho)
		else
			print("User dictionary: '"..userdict.."' is a directory and not a file")
		end
	end
end

scipm.orthospell.intercept_text = function(txt)  -- txt ~ allText
-- pre-spellcheck preparation of the document
	if editor.Lexer == 4  then   -- HTML
		txt = scipm.orthospell.strip_HTML (txt)
	elseif editor.Lexer == 14 or editor.Lexer == 49 then -- LaTeX or TeX
		txt = scipm.orthospell.strip_TeX(txt)
	end
	return txt
end

scipm.orthospell.intercept_word = function(at,wd,ws,wl) -- ~ allText, word, wstart, wlen
-- check and change word before it is spell-checked
	if editor.Lexer == 4 then -- HTML
		wd = string.gsub(wd,"xaxax","") -- remove soft hyphen substitute
	end
	return wd, ws, wl
end

scipm.orthospell.intercept_spellError = function(at,wd,ws,wl) -- ~ allText, word, wstart, wlen
-- if hunspell did not recognize the word, check if it is a special case such as
-- a character with several meanings (e.g. ' can be an apostrophe or a single quote)
-- you can then either drop it or check the modified word again. If such a case is rare
-- (e.g single quotes), then this is more economical then checking with intercept_word.
-- !! ws and wl MUST be returned by your function; check that wl is not nil before calling it!
-- !! return wl as nil if word was found to be correct
	if string.match(langChars,"-") then
		ws, wl = scipm.orthospell.check_hyphenation(wd,ws,wl)
	end
	if wl and string.match(langChars,"'") then -- check for single quote
		ws, wl = scipm.orthospell.check_singleQuote(wd,ws,wl)
	end
	if dictlang == "de" and  wl and wl < 6 then -- German specific
		ws, wl = scipm.orthospell.check_abbrivDot (at, wd, ws, wl)
	end
	return ws, wl
end

scipm.orthospell.intercept_suggestion = function(sp, ep, wd) -- ~ startPos, endPos, word
	if editor.Lexer == 4  and scipm.orthospell.check_softHyphen(sp, ep,wd) then
		return true
	end
	return false
end

-- end of customization

-- begin main

package.cpath = package.cpath .. ";" .. table.concat({scipm.data.path.scipmchild, "scipm.orthospell", "lib", "?.dll"}, scipm.data.path.sep)
if not shell then  -- if not the extman.lua is used that comes with Orthospell
	require ("shell")
end
if not hunspell then  -- if not the extman.lua is used that comes with Orthospell
    require("hunspell")
end

hspell = true
if not scite_FileExists(dictpath.."\\"..dictname..".aff") then
    print ("The dictionary "..dictname.." is not installed")
else
    hunspell.init(dictpath.."\\"..dictname..".aff", dictpath.."\\"..dictname..".dic")
    -- adding a user dictionary
    if userdict then scipm.orthospell.add_userDict() end
    scipm.orthospell.get_langParam(dictname)
    dicInit = true
end


scipm.orthospell.inline_spell = function()
    if (scite_GetProp("PLAT_WIN", nil) ~= nil) then
        if not dicInit then print ("No dictionary loaded!") return end
        if buffer["SpellMode"] then
            scipm.orthospell.reset_page()
            return
        end
        buffer["SpellMode"] = true
        cpMode = scipm.orthospell.get_cpMode()
        spChars = pcall(scipm.orthospell.set_wordChars)  -- use a character set that suits natural languages
        wordRegEx = scipm.orthospell.create_regEx()
        editor:Colourise(1, -1)
        allText = editor:GetText()
        allText = scipm.orthospell.intercept_text(allText) -- prepare the document before spell-checking
        for word, wstart in scipm.orthospell.get_words(allText) do
            local wlen = string.len(word)
            word, wstart, wlen = scipm.orthospell.intercept_word(allText, word, wstart, wlen) -- edit word before spell-checking
            word = scipm.orthospell.check_codePage(word,toDic) -- change encoding if necessary
            if not hunspell.spell(word) then
                wstart, wlen = scipm.orthospell.intercept_spellError(allText, word, wstart, wlen) -- check special cases
                if (wlen) then scipm.orthospell.highlight_range(wstart-1, wlen) end
            end
        end
    else
        print("scipm.orthospell only works on Windows")
    end
end

scipm.orthospell.get_words = function (allText)
	local wstart, wstop, word = 0, 0, nil
	return function ()
		while true do
			wstart, wstop, word = string.find(allText, wordRegEx, wstop+1)
			if not wstart then
				return nil
			elseif ignoreCAPS and string.match(word, "%u%u") then
			else
				return word, wstart
			end
		end
	end
end

scipm.orthospell.spell_suggest = function()
    if (scite_GetProp("PLAT_WIN", nil) ~= nil) then

        if not buffer["SpellMode"] or editor:AutoCActive() then return false end
        editor.AutoCAutoHide = false
        local pos = editor.CurrentPos-2
        local indicator = editor:IndicatorValueAt(spellIndic,pos)
        if indicator == 0 then
            pos = editor.Anchor + 1
            indicator = editor:IndicatorValueAt(spellIndic,pos)
        end
        if indicator > 0 then
            local startPos = editor:IndicatorStart(spellIndic, pos)
            if  cpMode > 1 then
                startPos = scipm.orthospell.check_posUTF(startPos)
            end
            local endPos = editor:IndicatorEnd(spellIndic, pos)
            local word = editor:textrange(startPos,endPos)
            if scipm.orthospell.intercept_suggestion(startPos, endPos, word) then return false end
            if  not hunspell.spell(word) then
                editor:SetSel(startPos, endPos)
                local wlen = string.len(word)
                word = scipm.orthospell.check_codePage(word,toDic)
                local sug = hunspell.suggest(word)
                if #sug > 0 then
                    local sugstr = table.concat(sug, " ")
                    sugstr = scipm.orthospell.check_codePage(sugstr,toDoc) -- change encoding if necessary
                    editor.AutoCSeparator = 32
                    editor:AutoCShow(wlen,sugstr)
                end
            end
        end
        return true
    else
        print("scipm.orthospell only works on Windows")
    end
end
-- end main

-- functions to be called by scipm.orthospell.intercept_text

scipm.orthospell.strip_HTML = function(txt)
-- replace stylesheets with blanks
	txt = string.gsub(txt, "<style.-</style>", function (w)
		return string.rep(" ",string.len(w))
		end)
-- replace scripts with blanks
	txt = string.gsub(txt, "<script.-</script>", function (w)
		return string.rep(" ",string.len(w))
		end)
-- replace tags with blanks
	txt = string.gsub(txt, "<.->", function (w)
		return string.rep(" ",string.len(w))
		end)
-- substitute &shy; entity with a string of (meaningless) letters
	txt = string.gsub(txt, "&shy;", "xaxax")
-- replace HTML entities with blanks
	txt = string.gsub(txt, "&%a-;", function (w)
		return string.rep(" ",string.len(w))
		end)
	return txt
end

scipm.orthospell.strip_TeX = function(txt)
-- replace commands with blanks
	txt = string.gsub(txt, "\\[^{%s]+", function (w)
		return string.rep(" ",string.len(w))
		end)
	-- Note: if you want to exclude text within curly brackets, change within the
	-- regEx the opening curly bracket to a closing one ("\\[^}%s]+")
	return txt
end

-- end functions scipm.orthospell.intercept_text

-- functions to be called by intercept_word
	-- empty
-- end intercept_word functions


-- functions to be called by scipm.orthospell.intercept_spellError

scipm.orthospell.check_hyphenation = function(wd, ws, wl)
-- skip word if it consists of hyphens only
	if string.find(wd,"^[%-]+$") then
		return ws, nil
	end
	return ws, wl
end

scipm.orthospell.check_singleQuote = function(wd, ws, wl)
-- check if the word is preceded or followed by a single quote and check again
	a1, wd, a2 =  string.match(wd, "^('?)(.-)('?'?)$")
	if a1 ~= "" or a2 ~= "" then
		if not hunspell.spell(wd) then
			return ws + string.len(a1), wl - string.len(a1) - string.len(a2)
		else
			return ws, nil -- meaning: word is correct
		end
	end
	return ws, wl
end

scipm.orthospell.check_abbrivDot = function(at, wd, ws, wl)
--	look ahead: the German dict requires the final dot in abbreviations
	local pos = ws + wl
	if string.sub(at,pos,pos)== "." then
		if not hunspell.spell(wd..".") then
			local fl = string.sub(at,pos+2,pos+2)
			if not string.match(fl, "%u") then wl = wl + 1
		end
		else
			return nil -- meaning: word is correct
		end
	end
	return ws, wl
end

-- end scipm.orthospell.intercept_spellError functions


-- functions to be called by scipm.orthospell.intercept_suggestion

scipm.orthospell.check_softHyphen = function(sp, ep, wd)
-- if the word contains soft hyphens, then show the plain word instead of suggestions
	if string.find(wd,"&shy;") then
		editor:SetSel(sp, ep)
		editor:AutoCShow(string.len(wd),"="..string.gsub(wd, "&shy;", ""))
		return true
	else return false
	end
end
-- end function scipm.orthospell.intercept_suggestion

scipm.orthospell.get_cpMode = function()
-- determine code page of document and dictionary
-- mode: 0 = iso iso, 1 = iso utf, 2 = utf iso, 3 = utf utf
	local mode = 0
	if editor.CodePage == 65001 then mode = 2 end
	if dictCP == "utf" then mode = mode + 1 end
	return mode
end

scipm.orthospell.check_codePage = function(str,to)
-- convert to or from utf-8
		if cpMode == 2 then -- utf iso
			if to == toDic then
				str = shell.from_utf8 (str)
			else
				str = shell.to_utf8(str)
			end
		elseif cpMode == 1  then -- iso utf
			if to == toDic then
				str = shell.to_utf8(str)
			else
				str = shell.from_utf8(str)
			end
		end
	return str
end

scipm.orthospell.set_wordChars = function()
-- the new character set does not directly affect spell checking. It may change,
-- however, what is highlighted if you double-click on a word. The new behaviour is:
-- accented characters are treated as word characters, wheras numbers are not
	if editor.Lexer == 0 then
		txtChars = editor.WordChars
	elseif editor.Lexer == 4 then
		htmlChars = editor.WordChars
	elseif editor.Lexer == 14 then
		texChars = editor.WordChars
	elseif editor.Lexer == 49 then
		texChars = editor.WordChars
	else return
	end
	editor.WordChars = spellChars..langChars
end

scipm.orthospell.reset_wordChars = function()
-- the original character set is restored
	if editor.Lexer == 0 then
		editor.WordChars = txtChars
	elseif editor.Lexer == 4 then
		editor.WordChars = htmlChars
	elseif editor.Lexer == 14 then
		editor.WordChars= texChars
	elseif editor.Lexer == 49 then
		editor.WordChars = texChars
	end
end

scipm.orthospell.create_regEx = function ()
-- creates the regular expression for the word search
	local wreg = "(["
	local lngReg = string.gsub(langChars,"%-","%%-")
	if cpMode > 1 then  -- document is UTF-8
		wreg = wreg..utfChars..shell.to_utf8(lngReg)
	else
		wreg = wreg..isoChars..lngReg
	end
	return wreg.."]+)"
end

scipm.orthospell.check_posUTF = function(sp)
-- correct start position for UTF-8 chars starting with byte c2
	if string.sub(allText,sp,sp) == "¬" then
		sp = sp + 1
	end
	return sp
end

scipm.orthospell.change_dict = function (nr)

 -- change the current dictionary
	scipm.orthospell.reset_page()
	local old_dict = dictname
	dictname = dictlist[tonumber(nr)]
	if not scite_FileExists(dictpath.."\\"..dictname..".aff") then
		print ("The dictionary "..dictname.." is not installed")
		dictname = old_dict
	else
		scipm.orthospell.get_langParam(dictname)
		hunspell.init(dictpath.."\\"..dictname..".aff", dictpath.."\\"..dictname..".dic")
		hunspell.add_dic(ortho)
		dicInit = true
        print('>> OK set dictionary to "' .. scipm.orthospell.get_language(scipm.orthospell.data.dictlist[nr]) .. '"')

	end
end

scipm.orthospell.highlight_range = function(pos, len)
	editor.IndicatorCurrent = spellIndic
	editor:IndicatorFillRange(pos, len)
end

scipm.orthospell.reset_page = function()
	editor.IndicatorCurrent = spellIndic
	editor:IndicatorClearRange(0, editor.TextLength)
	if spChars then scipm.orthospell.reset_wordChars() end
	buffer["SpellMode"] = false
end

scite_OnDoubleClick(scipm.orthospell.spell_suggest)

-- show list dictionary to output
-- -------------------------------------------------------------------------------
scipm.orthospell.userdictionary = function ()
    if (scite_GetProp("PLAT_WIN", nil) ~= nil) then
        local ext, shtct = "", ""
        local message, luaDoString = "", "";
        if scipm.orthospell.data.dictlist[2] then

            print("-- orthospell -- double click dictionary to change")
            for i in ipairs(scipm.orthospell.data.dictlist) do
                flang = scipm.orthospell.get_language(scipm.orthospell.data.dictlist[i])
                if not scite_FileExists(scipm.orthospell.data.dictpath.."\\"..scipm.orthospell.data.dictlist[i]..".aff") then
                    print ("The dictionary "..scipm.orthospell.data.dictlist[i].." is not installed")
                else
                    message = "* " .. flang;
                    if flang == props["spell.dictlang"] then message = message .. " (current)" end
                    luaDoString = "scipm.orthospell.change_dict(" .. i .. ");editor:GrabFocus()"
                    scipm.core.PrintWithHiddenDoString(message, luaDoString)
                end
            end
        end
    else
        print("scipm.orthospell only works on Windows")
    end
end
